public class ListaDoble{
	...
	public void borrarUltimoNodo(){
		this.topBackward = (Node) this.topBackward.previous;
	...
	//**************************************************/
	public Node buscarNodoPorPosicion(int posicion){
		if (posicion < 0) {
			return null;
		} else {
			Node temp = this.topForward;
	...
	public void insertarAntesDelUltimo(String nombre){
		Node temp = new Node();
		temp.name = nombre;
		temp.next = this.topBackward;
		temp.previous = this.topBackward.previous;
		
		if (this.topBackward.previous != null) {
			this.topBackward.previous = temp;
		} else {
			this.topForward = temp;
		}
